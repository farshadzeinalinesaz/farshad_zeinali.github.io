...
ssize_t device_read(struct file *filp, char __user *buffer, size_t len,
		loff_t *offset);
ssize_t device_write(struct file *filp, const char __user *buffer, size_t len,
		loff_t *offset);
...
ssize_t device_read(struct file *filp, char __user *buffer, size_t len,
		loff_t *offset) {
	int length = min(BUFFER_LEN- *offset,len);
	if (length <= 0) {
		return 0;
	}
	if (copy_to_user(buffer, messagePointer + *offset, length)) {
		return -EFAULT;
	} else {
		*offset += length;
		return length;
...
ssize_t device_write(struct file *filp, const char __user *buffer, size_t len,
	...
	if (copy_from_user(messagePointer + *offset, buffer, length)) {
		return -EFAULT;
	} else {
		*offset += length;
		return length;
