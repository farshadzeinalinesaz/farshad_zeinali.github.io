/*
 ===============================================================================
 Driver Name		:		ldd_proj_9_cdev_blocking_nonblocking_io_operations
 Author			:		FARSHAD
 License			:		GPL
...
#include"ldd_proj_9_cdev_blocking_nonblocking_io_operations.h"
...
ssize_t device_read(struct file *filp, char __user *buffer, size_t size,
		loff_t *offset);
ssize_t device_write(struct file *filp, const char __user *buffer, size_t size,
		loff_t *offset);
long device_ioctl(struct file *filp, unsigned int ioctl_num,
...
static int __init device_init(void) {
	int result = SUCCESS;
	int reg_status = alloc_chrdev_region(&device_num, FIRST_MINOR, N_MINORS,
			DRIVER_NAME);
	if (reg_status) {
		result = reg_status;
...
int device_open(struct inode *inp, struct file *filp) {
	...
	/*if (device_private_data->device_open_count > 0) {
	 return -EBUSY;
	 }*/
...
ssize_t device_read(struct file *filp, char __user *buffer, size_t size,
	...
	int
			length =
					min(size,(size_t)(device_private_data->buff_write_p-device_private_data->buff_read_p));
	if (length <= 0) {
		//O_NONBLOCK==O_NDELAY
		...
		length= min(size,(size_t)(device_private_data->buff_write_p-device_private_data->buff_read_p));
	}
	if (copy_to_user(buffer, device_private_data->buff_read_p, length)) {
		return -EFAULT;
	} else {
		*offset += length;
		device_private_data->buff_read_p += length;
		return length;
	}
	return length;
}
ssize_t device_write(struct file *filp, const char __user *buffer, size_t size,
	...
	int ret_val;
	dev_private_data *device_private_data = filp->private_data;
	if (device_private_data->buff_write_p == NULL) {
		device_private_data->buff_write_p = device_private_data->buffer;
		device_private_data->buff_read_p = device_private_data->buffer;
	}
	...
	if (copy_from_user(device_private_data->buff_write_p, buffer, length)) {
		ret_val = -EFAULT;
		goto OUTPUT;
	} else {
		*offset += length;
		device_private_data->buff_write_p += length;
		ret_val = length;
		goto OUTPUT;
	}
	OUTPUT:
	//cleanup
	up(&device_private_data->dev_sem);
	is_data_available = 1;
