/*
 ===============================================================================
 Driver Name		:		ldd_proj_8_cdev_semaphore
 Author				:		FARSHAD
 License			:		GPL
...
#include"ldd_proj_8_cdev_semaphore.h"
...
int device_open(struct inode *inp, struct file *filp) {
	...
	device_private_data->message_pointer = device_private_data->message;
	return SUCCESS;
}
ssize_t device_read(struct file *filp, char __user *buffer, size_t size,
		loff_t *offset) {
	...
	if (copy_to_user(buffer, device_private_data->message_pointer + *offset,
			length)) {
		return -EFAULT;
	} else {
		*offset += length;
...
ssize_t device_write(struct file *filp, const char __user *buffer, size_t size,loff_t *offset) {
	...
	if (copy_from_user(device_private_data->message_pointer + *offset, buffer,length)) {
		ret_val= -EFAULT;
		goto OUTPUT;
	} else {
		*offset += length;
